#!/usr/bin/env python3
"""
AI Translation Script for PenguinLab — local incremental translation of Chinese Markdown docs.

Translates Chinese .md files in document/ to English, outputting to the
Docusaurus i18n folder structure (site/i18n/en/docusaurus-plugin-content-docs/current/).
...
# PenguinLab paths (relative to project root)
DOCS_DIR = 'document'
I18N_OUTPUT_DIR = Path('site') / 'i18n' / 'en' / 'docusaurus-plugin-content-docs' / 'current'
...
def load_terminology_glossary(project_root: Path) -> Dict[str, str]:
    ...
    glossary_path = project_root / 'scripts' / 'terminology.md'
...
the Linux kernel and embedded systems. Your task is to translate Chinese \
Markdown documentation into natural, idiomatic English.

## Context
This is PenguinLab, a Linux kernel learning station covering schedulers, \
memory management, filesystems, networking, driver development, the full \
embedded Linux stack, debugging, and virtualization. The content uses QEMU \
for hands-on practice on ARM32/ARM64/RISC-V/x86_64 architectures, based on \
kernel 6.19.
...
- Preserve the language specifier in fenced blocks (```c, ```bash, ```dts, etc.).
...
### Device Tree Blocks
- Preserve all DTS property names, node names, and compatible strings.
- Translate only inline comments (after //) if they contain Chinese.

### Diff/Patch Blocks
- Preserve all diff syntax (+, -, @@) and code.
- Translate only contextual prose around the diff.
...
- Keep all other fields (slug, prerequisites, next, difficulty, tags, \
architectures, kernel_version, sources, sidebar_position, etc.) unchanged.
...
- For kernel source references, keep the format: `path/to/file.c:line_number`
"""
...
# ---------------------------------------------------------------------------
# Output path computation — Docusaurus folder-based i18n
# ---------------------------------------------------------------------------

def get_output_path(input_path: Path, project_root: Path) -> Path:
    """Compute Docusaurus i18n output path for a given source file.

    Input:  document/tutorials/foundations/01-setup.md
    Output: site/i18n/en/docusaurus-plugin-content-docs/current/tutorials/foundations/01-setup.md
    """
...
def detect_changed_files(project_root: Path) -> List[Path]:
    """Detect changed .md files in document/ relative to main branch."""
    try:
...
def find_all_chinese_md(docs_root: Path) -> List[Path]:
    """Find all Chinese .md files in document/."""
    files = []
    for f in docs_root.rglob('*.md'):
        if any(part in ('images',) for part in f.parts):
            continue
        files.append(f)
    return sorted(files)
...
def main():
    parser = argparse.ArgumentParser(
        description='Translate PenguinLab Chinese Markdown docs to English (Docusaurus i18n)')
    ...
    group.add_argument('--all', action='store_true',
                       help='Translate all Chinese .md files in document/')
    ...
    done_counter = [0]  # mutable counter for progress tracking
